...
ssize_t safe_write(const int fdr, const char *s, const size_t len) {
    ...
    while (total_written < len) {
        ssize_t bytes_written = write(fdr, s + total_written, len - total_written);
        if (bytes_written > 0) {
...
ssize_t safe_read(const int fdr, char *s, const size_t len) {
    ...
    while (total_read < len) {
        ssize_t bytes_read = read(fdr, s + total_read, len - total_read);
        if (bytes_read > 0) {
...
ssize_t safe_pread(const int fdr, char *s, const size_t len, off_t offset) {
    ssize_t total_read = 0;
    while (total_read < len) {
        ssize_t bytes_read = pread(fdr, s + total_read, len - total_read, offset + total_read);
        if (bytes_read > 0) {
            total_read += bytes_read;
        } 
        else if (bytes_read == 0) {
